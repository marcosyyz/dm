...
/*	private $db_host    = "mysql.diretoriomogi.com.br"; // server name
	private $db_user    = "diretoriomogi03";          // user name
	private $db_pass    = "aGmrzl2112";          // password
	private $db_dbname  = "diretoriomogi03";          // database name
	...
	public function __construct($connect = true, $database = null, $server = null,
		...
		if ($database == null) $this->db_dbname  = $Ad->getDBname();
		if ($server   == null) $this->db_host    = $Ad->getHost();
		if ($username == null) $this->db_user    = $Ad->getUser();
		if ($password == null) $this->db_pass    = $Ad->getPass();
		if ($charset  == null) $this->db_charset = $Ad->getCharset();
		...
		if (strlen($this->db_host) > 0 &&
			strlen($this->db_user) > 0) {
				if ($connect) 
				  if (!$this->Open()) echo 'Nao conectou!';   
		}
   ...
   /**
	...
	public function GetHTML($showCount = true, $styleTable = null, $styleHeader = null, $styleData = null) {
		...
		if ($this->last_result) {
			if ($this->RowCount() > 0) {
				$html = "";
				if ($showCount) $html = "Total: " . $this->RowCount() . "<br />\n";
	...
	*/
	...
	public function GetXML() {
		...
		if (is_resource($this->last_result)) {
			...
			if ($this->Error()) {
				...
				$root->setAttribute('error', "Nenhuma Query foi executada.");
			}
		}
	...
	public function Row($optional_row_number = null) {
		$this->ResetError();
		if (! $this->last_result) {
			$this->SetError("Nenhum resultado", -1);
			return false;
	...
	private function SetError($errorMessage = "", $errorNumber = 0) {
		try {
			...
			if ($this->IsConnected()) {
				$this->error_desc = mysqli_error($this->mysql_link).$msg;
			} else {
				$this->error_desc = mysqli_connect_error().$msg;
			}
	...
	static public function SQLValue($value, $datatype = self::SQLVALUE_TEXT) {
		...
		switch (strtolower(trim($datatype))) {
			...
			case "datetime":
				if (self::IsDate($value)) {
					$return_value = "'" . date('Y-m-d G:i:s', strtotime($value)) . "'";
				} else {
					$return_value = "NULL";
				}
				break;
	...
	public function UpdateRows($tableName, $valuesArray, $whereArray = null) {
        ...
        public function NextId($tableName, $id_field, $search_value, $search_field){
            $last_id = $this->QuerySingleValue(" SELECT MAX(".$search_field.") FROM ".$tableName);
                         
            //echo $last_id;
            //echo "<br>"."SELECT MAX(".$id_field.") FROM ".$tableName;
            //die;
            //$sql = " SELECT ". $id_field." FROM ".$tableName." WHERE ".$id_field." = ".($id_value+1);
            
            while($search_value < $last_id){
                $sql = " SELECT ". $id_field." FROM ".$tableName." WHERE ".$search_field." > ".$search_value
        ...
        public function PriorId($tableName, $id_field, $search_value,$search_field ){            
            while($search_value > 0){
                //$sql = " SELECT ". $id_field." FROM ".$tableName." WHERE ".$search_field." = ".$search_value
                  //      ." ORDERBY  ".$search_field." DESC ";
                $sql = " SELECT ".$id_field." FROM ".$tableName." WHERE ATIVIDADE_ORDEM < ".$search_value
